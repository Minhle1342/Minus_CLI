...
export interface DynamicContextInputs {
  ...
  /** P1.46: Domain-level contract retained by the intent guardian. */
  domainContractContext?: string;
  /** P1.47: Pre-mutation falsification reminder for risky tasks. */
  paretoGateReminder?: string;
  /** P1.48: Request-footprint guidance generated by the latency controller. */
  latencyGuidance?: string;
  /** Bounded, evidence-backed context carried across an explicit phase transition. */
  phaseHandoff?: string;
  /** P1.495: Khối prompt khuyến khích chạy lệnh test qua run_command khi có nhiều hơn 2 lượt edit */
  testVerificationEncouragement?: string;
  /** P1.5: Risk-adjusted explore guidance / implement patch spec / verify gate. */
  ...
  arbitrate(
    ...
    const rawSources: RankedSource[] = [
      ...
      },
      {
        ...
        content: (inputs.completionDirective || '').trim(),
        priority: 0.9,
        allowTruncation: false,
      },
      {
      ...
      {
        key: 'hypothesisContext',
        name: 'Hypothesis Scratchpad (P1.45)',
        content: (inputs.hypothesisContext || '').trim(),
        priority: 1.45,
        allowTruncation: false,
      },
      {
        key: 'hypothesisGuidance',
        name: 'Hypothesis Guidance (P1.46)',
        content: (inputs.hypothesisGuidance || '').trim(),
        priority: 1.46,
        allowTruncation: false,
      },
      {
        key: 'domainContractContext',
        name: 'Domain Contract (P1.47)',
        content: (inputs.domainContractContext || '').trim(),
        priority: 1.47,
        allowTruncation: false,
      },
      {
        key: 'paretoGateReminder',
        name: 'Pareto Gate Reminder (P1.48)',
        content: (inputs.paretoGateReminder || '').trim(),
        priority: 1.48,
        allowTruncation: false,
      },
      {
        key: 'latencyGuidance',
        name: 'Latency Guidance (P1.49)',
        content: (inputs.latencyGuidance || '').trim(),
        priority: 1.49,
        allowTruncation: false,
      },
      {
        key: 'testVerificationEncouragement',
        name: 'Test Verification Encouragement (P1.495)',
        content: (inputs.testVerificationEncouragement || '').trim(),
        priority: 1.495,
        allowTruncation: false,
      },
      {
      ...
      {
        key: 'phaseHandoff',
        name: 'Phase Handoff (P1.5)',
        content: (inputs.phaseHandoff || '').trim(),
        priority: 1.5,
        allowTruncation: false,
      },
      {
