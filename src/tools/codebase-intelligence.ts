...
export class CodebaseIntelligenceService {
  ...
  queryCallGraph(
    ...
    if (direction === "callees" || direction === "both") {
      if (targetFile) {
        const found = this.extractCallees(
          cleanSymbol,
          targetFile,
          depth,
          pruneNoise,
    ...
    if (direction === "callers" || direction === "both") {
      const found = this.extractCallers(
        cleanSymbol,
        depth,
        pruneNoise,
        visitedCallers,
    ...
    for (const file of scannedFiles) {
      ...
      if (file.includes(path.join("app", "")) || file.includes("app/")) {
        const routeMatch = file.match(
          /[\\/]app[\\/](.*?)[\\/](route|page)\.(ts|js|tsx|jsx)$/,
        );
        if (routeMatch) {
          const routeSubPath =
            "/" +
            routeMatch[1].replace(/\\/g, "/").replace(/\[(.*?)\]/g, ":$1");
      ...
      const expressMethodRegex =
        /(?:app|router|server)\.(get|post|put|delete|patch|use|all)\s*\(\s*(['"`])([^'"`]+)\2\s*(?:,\s*([^,\n\)]+))*/gi;
      ...
      const nestControllerRegex =
        /@Controller\s*\(\s*(['"`])?([^'"`\)]*)\1?\s*\)/g;
      const nestMethodRegex =
        /@(Get|Post|Put|Delete|Patch)\s*\(\s*(['"`])?([^'"`\)]*)\2?\s*\)[\s\S]*?(?:async\s+)?([a-zA-Z0-9_$]+)\s*\(/g;

      let controllerMatch: RegExpExecArray | null;
      let prefix = "";
      if ((controllerMatch = nestControllerRegex.exec(content)) !== null) {
        prefix = controllerMatch[2]
          ? "/" + controllerMatch[2].replace(/^\//, "")
          : "";
        let methodMatch: RegExpExecArray | null;
        while ((methodMatch = nestMethodRegex.exec(content)) !== null) {
          const httpMethod =
            methodMatch[1].toUpperCase() as RouteEntry["method"];
          const subPath = methodMatch[3]
            ? "/" + methodMatch[3].replace(/^\//, "")
            : "";
  ...
  getSymbolContext360(
    symbolName: string,
    filePath?: string,
  ): SymbolContext360Result {
    const cleanSymbol = symbolName.trim();
    const memberSymbol =
      cleanSymbol.split(".").filter(Boolean).pop() || cleanSymbol;
    ...
    const callers = callGraph.callers.map((c) => ({
      name: c.name,
      file: c.file,
      line: c.line,
    }));
    const callees = callGraph.callees.map((c) => ({
      name: c.name,
      file: c.file,
      line: c.line,
    }));
    ...
    return {
      ...
      kind,
      file: defFile
        ? path.isAbsolute(defFile)
          ? this.workspace.toRelativePath(defFile)
          : defFile.replace(/\\/g, "/")
        : undefined,
      line: defLine,
  ...
  /**
   * 3.1. Batch View 360 độ cho nhiều Symbols đồng thời
   */
  getSymbolContext360Batch(
    symbols: Array<{ symbolName: string; filePath?: string }>,
  ): SymbolContext360Result[] {
    return symbols.map((s) =>
      this.getSymbolContext360(s.symbolName, s.filePath),
    );
  }
  ...
  getArchitectureTopology(
    ...
    const relFiles = scannedFiles.map((f) =>
      this.workspace.toRelativePath(f).replace(/\\/g, "/"),
    );
    ...
    const compilerOptions = this.tsService.getCompilerOptions
      ? this.tsService.getCompilerOptions()
      : {};
    ...
    for (const file of scannedFiles) {
      ...
      for (const specifier of specifiers) {
        ...
        if (isTsOrJs) {
          try {
            ...
            if (
              ...
              const relCandidate = this.workspace
        ...
        if (
          ...
          const resolvedRel = this.workspace
            .toRelativePath(resolvedPath)
            .replace(/\\/g, "/")
            .toLowerCase();
    ...
    const layers: Record<string, ArchitectureLayer> = {
      controller: {
        ...
        description: "HTTP endpoints, routers, routes, CLI entrypoints",
        files: [],
      },
      service: {
        name: "Service / Domain Layer",
        description: "Core business logic, agents, workflows, engines",
        files: [],
      },
      repository: {
        name: "Data / Repository Layer",
        description: "Database access, models, entities, schemas",
        files: [],
      },
      ui: {
        name: "UI / Presentation Layer",
        description: "Components, views, pages, layouts, screens, hooks",
        files: [],
      },
      tools: {
        name: "Tools / Integration Layer",
        description: "Agent tool implementations, external APIs, MCP",
        files: [],
      },
      config: {
        name: "Configuration Layer",
        description: "Configuration, environment, constants",
        files: [],
      },
      utils: {
        name: "Utility / Helper Layer",
        description: "Shared utility functions, formats, types",
        files: [],
      },
      test: {
        name: "Test Layer",
        description: "Test suites, mocks, assertions",
        files: [],
      },
      other: {
        name: "Other Modules",
        description: "Bootstrap and general modules",
        files: [],
      },
    };
    ...
    for (const [fromFile, toFiles] of Object.entries(dependencyGraph)) {
      ...
      for (const toFile of toFiles) {
        ...
        if (
          ...
          (toLayer === "controller" || toLayer === "tools" || toLayer === "ui")
        ) {
        ...
        if (
          fromLayer === "config" &&
          (toLayer === "service" || toLayer === "controller")
        ) {
          layerViolations.push({
            from: fromFile,
            to: toFile,
    ...
    for (const file of relFiles) {
      ...
      const instability =
        sum === 0 ? 0 : Number((efferentCoupling / sum).toFixed(2));
    ...
    const avgInstability =
      relFiles.length === 0
        ? 0
        : Number((totalInstability / relFiles.length).toFixed(2));
    ...
    const mostStableModules = [...allMetrics]
      ...
      .sort(
        (a, b) =>
          a.instability - b.instability ||
          b.afferentCoupling - a.afferentCoupling,
      )
      .slice(0, 5);
    ...
    const mostUnstableModules = [...allMetrics]
      ...
      .sort(
        (a, b) =>
          b.instability - a.instability ||
          b.efferentCoupling - a.efferentCoupling,
      )
      .slice(0, 5);
  ...
  private extractCallees(
    ...
    if (!symbolBodyNode) {
      ...
      const declPattern = new RegExp(
        `(?:class|interface|struct|def|void|int|string|float|bool|async|public|private|protected)\\s+${symbolName}\\b`,
        "i",
      );
      let inScope = false;
      let braceCount = 0;
      for (let i = 0; i < lines.length; i++) {
        ...
        if (inScope) {
          ...
          for (const m of callMatches) {
            ...
            if (
              calledName &&
              calledName !== symbolName &&
              !keywords.has(calledName) &&
              !calls.has(calledName)
            ) {
            ...
            if (!isNoise || !pruneNoise) {
              if (
                !callers.some(
                  (c) =>
                    c.name === callerName &&
                    c.file === this.workspace.toRelativePath(file) &&
                    c.line === line + 1,
                )
              ) {
                callers.push({
      ...
      if (
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (
            ...
            !l.includes(`interface ${symbolName}`)
          ) {
  ...
  private findCircularCycles(
    ...
    const canonicalizeCycle = (nodes: string[]): string[] => {
      ...
      const pathNodes =
        nodes[0] === nodes[nodes.length - 1] ? nodes.slice(0, -1) : [...nodes];
      let minIdx = 0;
      ...
      const rotated = [
        ...pathNodes.slice(minIdx),
        ...pathNodes.slice(0, minIdx),
      ];
      ...
      const scan = (current: string) => {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            if (
              /\.(ts|tsx|js|jsx|py|go|rs|java|cpp|c|cs|shader|hlsl)$/i.test(
                entry.name,
              )
            ) {
              results.push(full);
