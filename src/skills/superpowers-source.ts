...
export const SUPERPOWERS_BUILTIN_SKILLS: Omit<SkillManifest, 'path' | 'source'>[] = [
  ...
  },
  {
    ...
    name: 'Git Operations',
    version: '1.0.0',
    description: 'Execute Git CLI operations through run_command with explicit argv (e.g. run_command "git status", "git diff", "git add <file>", "git commit -m <msg>"). Read-only commands are safe to inspect; write, network, and destructive commands must match the explicit current user request and pass the same scope/intent policy enforced inside run_command.',
    priority: 75,
  ...
  {
    id: 'game-development',
    name: 'Game Development & 2D Architecture',
    version: '1.0.0',
    description: 'Core principles of 2D and pixel game development: fixed timestep game loop, state machines, input abstraction, object pooling, and 60 FPS performance budgets.',
    priority: 30,
    requiredCapabilities: ['filesystem.read', 'filesystem.edit'],
    tags: ['game-development', 'game-programming', '2d-games', 'pixel-art', 'gated-game-skill'],
  },
  {
    id: 'unity-ai-game-creator',
    name: 'Unity AI Game Creator & Asset Pipeline',
    version: '1.0.0',
    description: 'End-to-end 5-phase game creation pipeline: GDD, scene blueprints, AI asset prompts for 2D/3D/audio, and script architecture.',
    priority: 35,
    requiredCapabilities: ['filesystem.read', 'filesystem.edit'],
    tags: ['unity', 'game-creator', 'game-pipeline', 'asset-prompts', 'gated-game-skill'],
  },
  {
